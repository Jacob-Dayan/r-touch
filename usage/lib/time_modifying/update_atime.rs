...
fn main() -> io::Result<()> {
    ...
    let file_secs = file_access_time.duration_since(UNIX_EPOCH).unwrap().as_secs();
...
#[cfg(test)]
mod tests {
    use super::update_access_time_of_file;
    ...
    fn atime_matches_yesterday() {
        ...
        update_access_time_of_file(path.to_str().unwrap(), "yesterday").unwrap();
