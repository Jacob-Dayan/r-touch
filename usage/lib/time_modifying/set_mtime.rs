...
//!
//! [`rtouch::set_modification_time`] changes only the modification
...
fn main() -> io::Result<()> {
    ...
    rtouch::set_modification_time(&path, three_days_ago)?;
    println!(
...
mod tests {
    ...
    fn mtime_is_updated() {
        ...
        rtouch::set_modification_time(&path, target).unwrap();
