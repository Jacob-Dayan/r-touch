...
//!
//! [`rtouch::datetime::parse_time_expression`] converts human-readable
//! strings such as `"yesterday"` or `"2 days ago"` into a [`std::time::SystemTime`],
//! which can then be passed directly to [`rtouch::touch`].

use std::io;

fn touch_with_expr(path: &str, expr: &str) -> io::Result<rtouch::ReplResult> {
    let time = rtouch::datetime::parse_time_expression(expr)
        .map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, e.to_string()))?;
    rtouch::touch(path, false, Some(time), true, false)
}
...
mod tests {
    ...
    fn yesterday_sets_atime_to_24h_ago() {
        ...
        let expected = rtouch::datetime::parse_time_expression("yesterday").unwrap();
        let diff = if atime > expected {
            atime.duration_since(expected)
        } else {
